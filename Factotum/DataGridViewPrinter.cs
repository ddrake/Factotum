...
class DataGridViewPrinter
{
    ...
    private int mColumnPoint;
	 private const int maxChars = 125;
            ...
            for (int i = 0; i < TheDataGridView.Columns.Count; i++)
            {
                ...
                for (int j = 0; j < TheDataGridView.Rows.Count; j++)
                {
                    ...
                    RowsHeight.Add(tmpSize.Height);
						  ...
						  trimmed = TheDataGridView.Rows[j].Cells[i].EditedFormattedValue.ToString();
						  // We're only displaying a single line, so replace newlines with spaces...
						  trimmed = trimmed.Replace("\n", "  ");
						  if (trimmed.Length > maxChars) trimmed = trimmed.Substring(0, maxChars - 3) + "...";
                    tmpSize = g.MeasureString(trimmed, tmpFont);
    ...
    private void DrawHeader(Graphics g)
    {
        ...
        if (IsWithTitle)
        {
            ...
            g.DrawString(TheTitleText, TheTitleFont, new SolidBrush(TheTitleColor), TitleRectangle, TitleFormat);
			  ...
			  int extraSpace = 6;
            CurrentY += g.MeasureString(TheTitleText, TheTitleFont).Height + extraSpace;
        }
        ...
        while (CurrentRow < TheDataGridView.Rows.Count)
        {
            if (TheDataGridView.Rows[CurrentRow].Visible) // Print the cells of the CurrentRow only if that row is visible
            {
                ...
                for (int CurrentCell = (int)mColumnPoints[mColumnPoint].GetValue(0); CurrentCell < (int)mColumnPoints[mColumnPoint].GetValue(1); CurrentCell++)
                {
                    ...
                    // Printing the cell text
						  string trimmed = TheDataGridView.Rows[CurrentRow].Cells[CurrentCell].EditedFormattedValue.ToString();
						  // We're only printing a single line, so replace newlines with spaces 
							trimmed = trimmed.Replace("\n", "  ");
						  if (trimmed.Length > maxChars) trimmed = trimmed.Substring(0, maxChars - 3) + "...";
						  g.DrawString(trimmed, RowFont, RowForeBrush, CellBounds, CellFormat);
